...
public class BossScript : MonoBehaviour
{
    ...
    IEnumerator Attack(Bounds bounds)
    {
        ...
        {
            Instantiate(hitmarker2, new Vector3(transform.position.x, 0, transform.position.z), Quaternion.Euler(0, (360 / howMany * i), 0));
        }
        yield return new WaitForSeconds(Random.Range(1, 3));
        //attack 3
        ...
        for (int i = 0; i < 6; i++)
        {
            GameObject curHitmarker = Instantiate(hitmarker3, new Vector3(transform.position.x, 0, transform.position.z - spawnInt * i), transform.rotation);
    ...
    IEnumerator Attack1(Bounds bounds)
    {
        ...
        if (randomNum == 0)
        {
            for (int i = 0; i < howMany; i++)
            {
                Vector3 loc = new Vector3(Random.Range(bounds.min.x, bounds.max.x), 0, Random.Range(bounds.min.z, bounds.max.z));
        ...
        else if (randomNum == 1)
        {
            ...
            for (int i = 0; i < howMany; i++)
            {
                Vector3 loc = new Vector3(player.transform.position.x, 0, player.transform.position.z);
                Instantiate(hitmarker, loc, Quaternion.identity);
                yield return new WaitForSeconds(Random.Range(1, 3));
            }
        }
